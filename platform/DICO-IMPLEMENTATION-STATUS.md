...
| Phase 4 — Admin Home Golden Screen | **NOT STARTED** | No hay checkpoint de Golden Screen ni aprobación humana. | — | No ejecutado. | Depende de Panorama y de aprobación visual. | **Al 3/sep el lado Panorama ya está cerrado** (Phase 8 CLOSED, Phase 9 V1 CLOSED). Queda **un solo bloqueo**: el gate A3 de Phase 3B y su commit de cierre, que verificado con `git log --all` no existe en ninguna rama. Cerrado A3, Golden Screen queda desbloqueada. |
...
| Phase 8 — Dico Native 2D | **CLOSED / CERTIFIED** (3/sep/2026, `98db946`). Historial del cierre por lotes: | Pose nativa, cara modular y presencia separada de avisos integradas. B1 agrega `DicoPresence` como autoridad única. B2 confirma que `DicoSlot` es controlado. B3 ubica la burbuja en flujo arriba de Native; B4 conserva el typewriter y reserva su geometría completa desde el primer frame con una sola fuente accesible. `9741b96` fue auditado como equivalente conceptual de `0304f28`; se conserva una sola implementación. | `0304f28`, recovery `67e95ce`, B1 `be5d6b5`, B3 `f429722`, B4 `f33d8eb` | 6 archivos / 61 tests dirigidos; suite 77 / 1.044 PASS. QA same-ref: DOM 8/8, screenshots 8/8, `blockingDiffPixels: 0`, scroll 40/40; geometría estable en 320/375/768/1440. | Queda la discrepancia de escala Native/Physical: misma anatomía, pero en Physical los ojos miden 10,21 % del ancho de la moneda contra 23,24 % en Native. Es acabado, no fuente: pertenece al lote de Physical o a B6. **Nota 3/sep:** el pack 3D oficial reemplazó el asset Physical viejo (cuerpo sin cara + capa de tinta); esta discrepancia **no se volvió a medir** contra el pack nuevo, así que sigue anotada como pendiente de verificación, no como resuelta. | B6 ejecutado y cerrado. Native 2D no requiere más lotes; cualquier ajuste de acabado entra por el cierre general de primitives/presencia. |
| Phase 9 — Slot / Physical (V1) | **CLOSED / QA CERTIFIED** (3/sep/2026) | Physical pasó de pack de assets a runtime conectado. `DicoPhysical` (crossfade 140ms, sin morphing/rig/lipsync), caja 448×318,08 derivada del contrato 1600×1136 conservando el tamaño en pantalla del Dico anterior; anclaje por **tinta** medida, no por caja. Event contract auditado (`DICO-PHYSICAL-EVENT-CONTRACT.md`) → **dos** eventos. `intervenciones.js` puro, recibe EVENTO y no estado. Dos placements semánticos (`presence`/`target`), `target` por portal a un nodo real y exclusivo de poses direccionales. Responsive: bajo 769px `catalogo-vacio` **no monta Physical** y conserva Native 2D, sin degradar `pointDown` ni cambiar de placement. | `0c67a14`, `4086d57`, `dfacae2`, `371617c`, `71b12ce`, `b50efab`, `613aae4`, `28ac616`, cierre **`98db946`** | Suite 1172/1172; harness 28/28; validator 3D 8/8·8/8·21/21; `phase9-visual` (7 viewports + 769 + cruce 768↔769 en ambos sentidos), `dico-intervenciones` y `dico-physical-poses` verdes en Chromium real; **same-ref 5/5** con DOM igual, píxeles bloqueantes 0, scroll identical y red externa 0; `git diff --check` limpio. | **Deuda visual del Slot: NO BLOQUEANTE** — con anclaje `target` la ranura queda visualmente entre el personaje y el CTA; se evalúa en el cierre general de primitives/presencia, no antes. `DicoPresence` se remonta al cruzar 769px (la carga vive en el productor, que no se remonta). | **No hay tercer evento Physical**: sólo `catalogo-vacio` y `nada-visible`. `margen-negativo` no implementado; `success`/`error`/`thinking`/`pointUp` existen en el pack y no en el runtime — no buscarles uso. Agregar un evento es decisión de producto, no de implementación. |
