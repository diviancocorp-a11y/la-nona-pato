...
| B6 — expresiones canónicas | **CLOSED** | `platform/PHASE-B6-EXPRESSIONS.md`. Siete estados emocionales + tres frames de habla sobre una sola anatomia. Nuevos `pensando` y `error`; los cinco previos no se reimplementaron. `error` usa la FORMA (X sobre la esclera) y el rojo solo como acento. Physical dejo de estar clavado en `idle`. Proporcion Physical corregida en el wrapper: de 2,28x de desvio a menos de 12,5%, jerarquia ojos/boca identica. 18 contratos, 9 mutaciones verificadas. Pendiente: ningun codigo emite `pensando`, `contento` ni `error` todavia. |
