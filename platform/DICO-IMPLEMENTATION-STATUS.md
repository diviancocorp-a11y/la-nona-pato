...
| Phase 8 — Dico Native 2D | **IN PROGRESS** | Pose nativa, cara modular y presencia separada de avisos integradas. `9741b96` fue auditado como equivalente conceptual de `0304f28`: CSS y test son blobs idénticos; se conserva el WebP funcional más nuevo de `0304f28` (`aaa84f5`, 23.956 bytes) y sólo la aclaración documental del recovery. | `0304f28`, recovery `67e95ce`, equivalencia `9741b96` | Tests Dico dirigidos del checkpoint Stage A. | Todavía no existe el orquestador único `DicoPresence`; Native y Physical aún no tienen exclusión centralizada. | Detenerse antes de `DicoPresence` hasta cerrar este checkpoint y sus gates. |
| Phase 9 — Slot / Physical | **IN PROGRESS** | Frontera física, asset Physical, retorno completo, reserva de espacio durante cierre y avisos en flujo integrados desde el recovery. | `a3b07bc`, `de5568a`, `7f40419`, `7b17e97` | Tests `dicoSlot`, `dicoAvisos`, `machineSoul` y gate integrado de Stage A. | Falta validación real de proporción, clipping y transición Native↔Physical. | Cerrar primero Stage A; luego centralizar presencia en Stage B. |
...
Los resultados finales de gates se registran en el HANDOFF del mismo
checkpoint. Este documento no autoriza avanzar a `DicoPresence` ni a Panorama.
